...
| ID | Description | Impact | How to Validate |
|---|---|---|---|
| 1 | UserRolesForSingleUser for privileged (directory) roles now handles users with no privileged role assignments | Prevents `SQLite Error 1: 'no such table: UserRolesForSingleUser'` during single user refresh for users without any directory role assignments | Run single user refresh for a user that has no Entra directory roles and verify the operation completes successfully with zero roles returned |
---
